(ns livestock-dairy.actor
  "LivestockDairyActor — the ISCO-08 6121 independent livestock-and-
  dairy actor as a `langgraph.graph/state-graph` (per ADR-2607011000
  / CLAUDE.md Actors section). One graph run = one feed/monitor-health
  ...
  an interrupted run can resume after human sign-off.
  ...
  The unconditional invariant: the LivestockAdvisor can never
  directly commit a record or dispatch a robot action the
  LivestockDairyGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [livestock-dairy.advisor :as advisor]
            [livestock-dairy.governor :as governor]
            [livestock-dairy.store :as store]))

(defn build-graph
  "Build a compiled LivestockDairyActor graph. `store` implements
  `livestock-dairy.store/Store`. `advisor` implements
  `livestock-dairy.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:herd-id (:herd-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
